...
static uint8_t cs_pin = SPI_SS;     // chip select pin
static uint8_t irq_pin = RFM_IRQ;   // Pin connected to IRQ
static uint8_t irq_num = 0;         // The interrupt number of the pin
...
void rf12_set_irq(uint8_t pin, uint8_t num)
{
  irq_pin = pin;
  irq_num = num;
}
...
static uint8_t rf12_byteCS (uint8_t out) {
  digitalWrite(cs_pin, LOW);

  uint8_t r = rf12_byte(out);
  digitalWrite(cs_pin, HIGH);
  return r;
}
...
#ifdef EIMSK
    // bitClear(EIMSK, INT0);
    bitClear(EIMSK, INT0 + irq_num);
    uint16_t r = rf12_xferSlow(cmd);
    // bitSet(EIMSK, INT0);
    bitSet(EIMSK, INT0 + irq_num);
#else
    // ATtiny
    bitClear(GIMSK, INT0);
    uint16_t r = rf12_xferSlow(cmd);
...
uint8_t rf12_canSend () {
    ...
    if (rxstate == TXRECV && rxfill == 0 &&
            (rf12_byteCS(0x00) & (RF_RSSI_BIT >> 8)) == 0) {
...
uint8_t rf12_initialize (uint8_t id, uint8_t band, uint8_t g) {
    ...
    if ((nodeid & NODE_ID) != 0)
        ...
        attachInterrupt(irq_num, rf12_interrupt, LOW);
    else
        // detachInterrupt(0);
